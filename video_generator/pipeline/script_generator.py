...
class ScriptGenerator:
    ...
    def _validate_duration(self, script: TikTokScript) -> TikTokScript:
        ...
        if script.total_duration < min_duration:
            logger.warning(f"Script duration ({script.total_duration}s) is below minimum ({min_duration}s). Adjusting...")
            ...
            logger.warning(f"Script duration ({script.total_duration}s) exceeds maximum ({max_duration}s). Adjusting...")
    ...
    def generate_script(self, processed_input: Dict[str, Any]) -> TikTokScript:
        ...
        original_content = processed_input["original_content"]
        ...
        messages = [
            SystemMessage(content=f"You are an expert {self.config.platform} scriptwriter."),
        ...
        try:
            ...
            logger.info(f"Generated script with {len(script.segments)} segments, {script.total_duration}s total duration")
            return script
    ...
    def quality_check_script(self, script: TikTokScript, processed_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.llm.structured_output(messages, output_schema)
            logger.info(f"Script quality check: score={result['quality_score']}, passes={result['passes_quality_check']}")
            return result
