...
export async function GET(
  ...
  try {
    ...
    if (isDebug) {
      console.log(`Attempting to fetch audio for task ${taskId}, scene ${sceneIndex}`);
    }
    ...
    for (const backendUrl of backendUrls) {
      try {
        const audioUrl = `${backendUrl}/media/audio/${taskId}/${sceneIndex}`;
        
        if (isDebug) {
          console.log(`Trying backend URL: ${audioUrl}`);
        }
